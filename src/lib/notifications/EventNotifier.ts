...
export class EventNotifier {
  ...
  private async refresh() {
    ...
    for (let [key, value] of Object.entries(events)) {
      let cache = await this.client.cache.json.get(`events:${this.client.user?.id}:${key}`);

      const cachedEvent = cache! as Event;

      if (!cache || cachedEvent.timestamp !== value.timestamp) {
        await this.client.cache.json.set(`events:${this.client.user?.id}:${key}`, ".", value);
